...
int main() {
   std::ifstream stream("./vmax_1000.dat");
   string line;
   ...
   while( i < n ){
      ...
      i++;
      r_close = 2.0 / ((data[i][0]*vel*sqrt(1 - Z*Z))*(data[i][0]*vel*sqrt(1 - Z*Z)));

      if( r_close > r_min ){
         std::cout << std::setprecision(15);
         std::cout << v[n].x << " " << v[n].y << " " << v[n].z << " " << vel << std::endl;
         //std::cout << i << " " << data[i][2] << " " << data[i][3] << " " << data[i][4] << " " << v[n].x << " " << v[n].y << " " << v[n].z << " " << "0" << std::endl;
      }
   }
